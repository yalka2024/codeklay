...
export function JiraSettings() {
  ...
  async function fetchStatus() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/enterprise/jira?orgId=${orgId}`);
  ...
  async function handleConnect() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/enterprise/jira', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'connect', orgId, apiKey }),
      });
      const data = await res.json();
  ...
  return (
    <div>
      ...
      <p>Connect your Jira account to automate issue tracking and project management.</p>
      ...
      {status !== 'connected' && (
        <div style={{ margin: '8px 0' }}>
          <input
            ...
            style={{ marginRight: 8 }}
          />
          <button onClick={handleConnect} disabled={loading || !apiKey}>Connect</button>
        </div>
      )}
      {status === 'connected' && (
        <div style={{ margin: '8px 0' }}>
