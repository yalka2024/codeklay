...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (mode) {
      ...
      case 'review':
        const reviewResult = await aiService.reviewCodeSecurity(
          messages[messages.length - 1].content,
          projectContext
        );
        
        response = {
          message: `Code Analysis Complete\n\nIssues Found: ${reviewResult.issues.length}\nSecurity Score: ${reviewResult.securityScore}%\nPerformance Score: ${reviewResult.performanceScore}%\nMaintainability Score: ${reviewResult.maintainabilityScore}%\nOverall Score: ${reviewResult.overallScore}%\n\n${reviewResult.issues.map(issue => `- ${issue.severity.toUpperCase()}: ${issue.message}`).join('\n')}\n\nSuggestions:\n${reviewResult.suggestions.map(s => `- ${s}`).join('\n')}`,
